...
public class FluxLoggingTests {
  ...
  public void logSimple() {
    ...
    System.out.println("leaving logSimple");
  }
  ...
  public void logMapping() {
    Flux<String> beltColors = Flux.just("white", "yellow", "orange", "green", "purple", "blue")
      .map(cb -> cb.toUpperCase()).log();
    beltColors.subscribe(); 
    System.out.println("leaving logMapping");
  }
  ...
  public void logFlatMapping() throws Exception {
    Flux<String> beltColors = Flux.just("white", "yellow", "orange", "green", "purple", "blue")
      .flatMap(cb -> Mono.just(cb)
          .map(c -> c.toUpperCase()).log()
          .subscribeOn(Schedulers.parallel()));
    
    beltColors.subscribe(); 
    System.out.println("leaving logFlatMapping");
    Thread.sleep(3000L);
  }
